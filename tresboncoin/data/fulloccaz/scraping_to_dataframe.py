        ...
        for html_file in [file for file in os.listdir(directory) if file.endswith(".html")]:
            ...
            if isinstance(url_, str):
                if add_the_announce(data, uniq_id_, price_):
                    ...
                    df["price"] = price_
                    df["city"] = announce_scrap_clear.get_city()
                    df["postal code"] = announce_scrap_clear.get_postal()
                    df["seller"] = announce_scrap_clear.get_seller()
                    df["seller_name"] = announce_scrap_clear.get_seller_name()
